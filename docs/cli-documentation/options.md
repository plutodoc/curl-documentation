...
_curl --cert certfile --key keyfile https://example.com_
...
If the cookie jar cannot be created or written to, the whole curl operation will not fail or even report an error clearly. Using --verbose will get a warning displayed, but that is the only visible feedback you get about this possibly lethal situation.
...
_curl --ftp-pret ftp://example.com/_
...
_curl --noproxy "www.example" https://example.com_
...
_curl --proto =http,https,sftp https://example.com_
...
_curl --proxy-ntlm --proxy-user user:passwd -x http://proxy https://example.com_
...
_curl --resolve example.com:443:127.0.0.1 https://example.com_
...
_curl --retry 5 --retry-all-errors https://example.com_
...
_curl --retry-connrefused --retry https://example.com_
...
_curl --retry-delay 5 --retry https://example.com_
...
_curl --retry-max-time 30 --retry 10 https://example.com_
...
_curl --retry 7 https://example.com_
...
_curl --sasl-authzid zid imap://example.com/_
...
_curl --sasl-ir imap://example.com/_
...
_curl --service-name sockd/server https://example.com_
...
_curl --show-error --silent https://example.com_
...
Since 7.52.0, --preproxy can be used to specify a SOCKS proxy at the same time --proxy is used with an HTTP/HTTPS proxy. In such a case curl first connects to the SOCKS proxy and then connects (through SOCKS) to the HTTP or HTTPS proxy.

If this option is used several times, the last one will be used.

Example:

_curl --socks4 hostname:4096 https://example.com_
...
_curl --socks4a hostname:4096 https://example.com_
...
_curl --socks5-basic --socks5 hostname:4096 https://example.com_
...
_curl --socks5-gssapi-nec --socks5 hostname:4096 https://example.com_
...
_curl --socks5-gssapi-service sockd --socks5 hostname:4096 https://example.com_
...
_curl --socks5-gssapi --socks5 hostname:4096 https://example.com_
...
_curl --socks5-hostname proxy.example:7000 https://example.com_
...
_curl --socks5 proxy.example:7000 https://example.com_
...
_curl --speed-limit 300 --speed-time 10 https://example.com_
...
_curl --ssl-allow-beast https://example.com_
...
_curl --ssl-auto-client-cert https://example.com_
...
_curl --ssl-no-revoke https://example.com_
...
_curl --ssl-reqd ftp://example.com_
...
_curl --ssl-revoke-best-effort https://example.com_
...
_curl --sslv2 https://example.com_
...
_curl --sslv3 https://example.com_
...
_curl --stderr output.txt https://example.com_
...
_curl --styled-output -I https://example.com_
...
_curl --suppress-connect-headers --include -x proxy https://example.com_
...
_curl --tcp-fastopen https://example.com_
...
_curl --tcp-nodelay https://example.com_
...
_curl -t TTYPE=vt100 telnet://example.com/_
...
_curl --tftp-blksize 1024 tftp://example.com/file_
...
_curl --tftp-no-options tftp://192.168.0.1/_
...
_curl -z "Wed 01 Sep 2021 12:18:00" https://example.com_

_curl -z "-Wed 01 Sep 2021 12:18:00" https://example.com_

_curl -z file https://example.com_
...
_curl --tls-max 1.2 https://example.com_

_curl --tls-max 1.3 --tlsv1.2 https://example.com_
...
_curl --tls13-ciphers TLS_AES_128_GCM_SHA256 https://example.com_
...
_curl --tlsauthtype SRP https://example.com_
...
_curl --tlspassword pwd --tlsuser user https://example.com_
...
_curl --tlsv1.0 https://example.com_
...
_curl --tlsv1.1 https://example.com_
...
_curl --tlsv1.2 https://example.com_
...
_curl --tlsv1.3 https://example.com_
...
_curl --tlsv1 https://example.com_
...
_curl --tr-encoding https://example.com_
...
_curl --trace-ascii log.txt https://example.com_
...
_curl --trace-time --trace-ascii output https://example.com_
...
_curl --trace log.txt https://example.com_
...
_curl --unix-socket socket-path https://example.com_
...
You can specify one --upload-file for each URL on the command line. Each -T, --upload-file + URL pair specifies what to upload and to where. curl also supports "globbing" of the --upload-file argument, meaning that you can upload multiple files to a single URL by using the same URL globbing style supported in the URL.
...
_curl -T file https://example.com_

_curl -T "img[1-1000].png" ftp://ftp.example.com/_

_curl --upload-file "{file1,file2}" https://example.com_
...
This option may be used any number of times. To control where this URL is written, use the --output or the --remote-name options.

**WARNING**: On Windows, particular file:// accesses can be converted to network accesses by the operating system. Beware!

Example:

_curl --url https://example.com_
...
_curl -B ftp://example.com/README_
...
_curl -A "Agent 007" https://example.com_
...
_curl -u user:secret https://example.com_
...
_curl --verbose https://example.com_
...
- redirect_url When an HTTP request was made without --location to follow redirects (or when --max-redirs is met), this variable will show the actual URL a redirect would have gone to.
...
- stderr From this point on, the --write-out output will be written to standard error. (Added in 7.63.0)

- stdout From this point on, the --write-out output will be written to standard output. This is the default, but can be used to switch back after switching to stderr. (Added in 7.63.0)
...
_curl --xattr -o storage https://example.com_
